  ...
  public:
	...
	void fillmydspSIG0(int count, float* table) {
		for (int i1 = 0; i1 < count; i1 = i1 + 1) {
			iRec0[0] = iRec0[1] + 1;
			table[i1] = std::exp(-(fConst0 * static_cast<float>(iRec0[1])));
			iRec0[1] = iRec0[0];
		}
	}
...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "f03_sr_dependent.dsp");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.9.0");
		m->declare("name", "f03_sr_dependent");
	...
	virtual void instanceConstants(int sample_rate) {
		fSampleRate = sample_rate;
		fConst1 = 2.0f / std::min<float>(1.92e+05f, std::max<float>(1.0f, static_cast<float>(fSampleRate)));
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("f03_sr_dependent");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iVec0[0] = 1;
			float fTemp0 = ((1 - iVec0[1]) ? 0.0f : fConst1 + fRec1[1]);
			fRec1[0] = fTemp0 - std::floor(fTemp0);
			output0[i0] = static_cast<FAUSTFLOAT>(ftbl0mydspSIG0[std::max<int>(0, std::min<int>(static_cast<int>(1024.0f * fRec1[0]), 1023))]);
			iVec0[1] = iVec0[0];
			fRec1[1] = fRec1[0];
		}
	}
