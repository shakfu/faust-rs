...
pub(crate) fn corpus_status_report() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Divergence classification (C3, `porting/mcp-server-analysis-and-plan-2026-07-21-en.md`):
    // splits the `ERR/OK` bucket above into cases where the C++ reference
    // merely lacks a feature `faust-rs` deliberately adds (`fad`/`rad`) versus
    // a genuine parity break. Reuses `classify_divergence` from
    // `corpus_status_query.rs` so this report and `corpus-status-query` can
    // never disagree on what counts as "expected".
    let mut expected_divergence = 0usize;
    let mut real_divergence = 0usize;
    ...
    for file in files {
        ...
        match classify_divergence(cpp.ok, rust.ok, &cpp.reason) {
            DivergenceClass::ExpectedDivergence => {
                expected_divergence = expected_divergence.saturating_add(1)
            }
            DivergenceClass::RealDivergence => real_divergence = real_divergence.saturating_add(1),
            DivergenceClass::OkOk | DivergenceClass::ErrErr => {}
        }
    ...
    if let Some(generated_at) = utc_timestamp_string() {
        writeln!(&mut out, "Generated at (UTC): `{generated_at}`")?;
    }
    writeln!(
        &mut out,
        "**Staleness note:** this document is a snapshot as of the timestamp above. \
         Prefer `cargo run -p xtask -- corpus-status-query --all` (or `--case <path>...` \
         for a subset) for a machine-readable, always-current answer that carries its own \
         generation timestamp and corpus size, rather than diffing this file by hand."
    )?;
    writeln!(&mut out)?;
    writeln!(&mut out, "## Inputs")?;
    writeln!(
        &mut out,
        "- Corpus: `tests/corpus/*.dsp` (`{total}` files at generation time)"
    )?;
    ...
    writeln!(
        &mut out,
        "  - of which **expected divergence** (C++ lacks a `faust-rs`-only feature, e.g. \
         `fad`/`rad`/`ondemand`; by design, not a regression): `{expected_divergence}`"
    )?;
    writeln!(
        &mut out,
        "  - of which **real divergence** (needs maintainer attention): `{real_divergence}`"
    )?;
    writeln!(&mut out)?;
    ...
    writeln!(
        &mut out,
        "| Case | Class | Divergence | C++ | Rust stage | Rust reason | C++ reason |"
    )?;
    writeln!(
        &mut out,
        "|------|-------|------------|-----|------------|-------------|------------|"
    ...
    for (case, cpp, rust) in &rows {
        ...
        let divergence = match classify_divergence(cpp.ok, rust.ok, &cpp.reason) {
            DivergenceClass::ExpectedDivergence => "expected",
            DivergenceClass::RealDivergence => "real",
            DivergenceClass::OkOk | DivergenceClass::ErrErr => "n/a",
        };
        writeln!(
            &mut out,
            "| `{}` | `{}` | `{}` | `{}` | `{}` | `{}` | `{}` |",
...
/// Stack size for the corpus-traversal compile thread.
///
/// The front-end recurses over the signal forest, and some corpus DSPs are deep
/// enough to exhaust the default main-thread stack: before this, both
/// `corpus-status-report` and `corpus-status-query --all` aborted the whole
/// process with `fatal runtime error: stack overflow` (exit 134) partway
/// through the corpus, producing no output at all. A per-case worker thread with
/// a generous stack keeps whole-corpus traversal usable.
///
...
///
/// The compile runs on a scoped worker thread with an enlarged stack; see
/// [`CORPUS_COMPILE_STACK_BYTES`].
pub(crate) fn rust_case_status(compiler: &compiler::Compiler, input: &Path) -> CaseStatus {
    std::thread::scope(|scope| {
        std::thread::Builder::new()
            .stack_size(CORPUS_COMPILE_STACK_BYTES)
            .spawn_scoped(scope, || rust_case_status_inner(compiler, input))
            .expect("failed to spawn corpus compile thread")
            .join()
            .unwrap_or_else(|_| CaseStatus {
                ok: false,
                stage: "panic",
                reason: "rust compile panicked".to_owned(),
            })
    })
}

/// Body of [`rust_case_status`], run on the enlarged-stack worker thread.
fn rust_case_status_inner(compiler: &compiler::Compiler, input: &Path) -> CaseStatus {
    match compiler.compile_file_default_to_signals(input) {
        ...
        Err(err) => {
            let (stage, reason) = match &err {
                ...
                compiler::CompilerError::UiLayout { .. } => ("ui", err.to_string()),
                ...
                compiler::CompilerError::CodegenCodebox { .. } => ("codegen", err.to_string()),
                compiler::CompilerError::CodegenCmajor { .. } => ("codegen", err.to_string()),
