...
#[test]
fn rejects_legacy_minput_modulation_form() {
    let output = parse_program(
        r#"process = minput("gain" : _).(_);"#,
        "slice12_mod_legacy_minput.dsp",
    );
    assert!(
        output.state.ctx.parse_error_count() > 0 || !output.errors.is_empty(),
        "legacy minput syntax should not parse as valid expression"
    );
}
