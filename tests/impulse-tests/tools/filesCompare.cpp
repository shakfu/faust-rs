...
{
    // faust-rs change: scan all trailing args for the optional tolerance so it
    // can be combined with the `-part` flag in any order. The upstream tool only
    // honored a tolerance in argv[3] when argc==4, which made `-part <tol>`
    // silently fall back to the default tolerance.
    double tolerance = 2e-06;
    for (int i = 3; i < argc; i++) {
        if (strcmp(argv[i], "-part") == 0) continue;
        double param = strtod(argv[i], NULL);
        // -1 (or any non-positive) value means "keep the default".
        if (param > 0) { tolerance = param; break; }
    }
