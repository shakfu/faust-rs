...
- `CPP_ORACLE_UNSUPPORTED` — generated by `make reference` after compiling the
  selected corpus with the configured C++ Faust oracle. These DSPs are excluded
  from every differential backend target because they have no C++ reference,
  but they are not faust-rs known failures. Their diagnostics remain in
  `build/ref/cpp-oracle-errors/<dsp>.log`; use `make reference-report` to list
